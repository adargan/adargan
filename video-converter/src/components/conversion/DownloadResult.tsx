  ...
  return (
    <div className="rounded-xl border border-green-200 bg-green-50 p-4 flex items-center gap-4">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-gray-900 truncate">{outputFilename}</p>
        <p className="text-xs text-gray-500 mt-0.5">
          {formatBytes(outputSize)}
          {savings !== null && savings > 0 && (
            <span className="ml-1 text-green-600 font-medium">· {savings}% smaller</span>
          )}
          {savings !== null && savings < 0 && (
            <span className="ml-1 text-orange-500 font-medium">· {Math.abs(savings)}% larger</span>
          )}
        </p>
      </div>
      <a
