...
export function ImageFormatControls() {
  ...
  return (
    <div className="space-y-5 rounded-xl border border-gray-700 bg-gray-900 p-4">
      <div>
        <p className="mb-2 text-sm font-medium text-gray-300">Output format</p>
          ...
          {FORMATS.map((fmt) => {
            ...
            return (
              ...
              >
                ...
                {disabled && <span className="ml-2 text-xs text-gray-600">(unsupported)</span>}
              </button>
            )
          })}
        </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <p className="text-sm font-medium text-gray-300">Quality</p>
          <span className="text-sm font-semibold text-gray-100">{quality}</span>
        </div>
        <input
          type="range"
          min={1}
